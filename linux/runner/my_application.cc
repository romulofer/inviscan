...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "inviscan");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "inviscan");
  }

  gtk_window_set_default_size(window, 1280, 720);

  // Ícone da janela/taskbar: instalado em <exe_dir>/data/app_icon.png pelo
  // CMake (o template Flutter Linux não define ícone algum).
